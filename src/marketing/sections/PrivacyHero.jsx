import mark from "../../assets/mark.png";
import styles from "./LegalHero.module.css";

const PrivacyHero = () => {
  return (
    <section className={styles.hero} aria-labelledby="privacy-hero-title">
      <img src={mark} alt="" aria-hidden="true" className={styles.mark} />
      <div className={styles.vignette} aria-hidden="true" />

      <div className={styles.inner}>
        <div className={styles.copy}>
          <h1 id="privacy-hero-title" className={styles.headline}>
            Your data is on the record.{" "}
            <em>Not ours to sell, share, or hold hostage.</em>
          </h1>
          <p className={`${styles.para} md-typescale-body-large`}>
            Kostody is built on the principle that you own the history of your
            repair. This page explains exactly what we collect, what we never
            touch, and who sees what.
          </p>
        </div>

        <div className={styles.sealWrap} aria-hidden="true">
          <div className={styles.seal}>
            <svg className={styles.ring} viewBox="0 0 200 200" role="presentation">
              <defs>
                <path
                  id="privacySealArc"
                  fill="none"
                  d="M100,100 m-74,0 a74,74 0 1,1 148,0 a74,74 0 1,1 -148,0"
                />
              </defs>
              <circle className={styles.ringAccent} cx="100" cy="100" r="95" />
              <circle className={styles.ringLine} cx="100" cy="100" r="89" />
              <circle className={styles.ringLine} cx="100" cy="100" r="58" />
              <text className={styles.ringText}>
                <textPath href="#privacySealArc" startOffset="0">
                  {"YOUR DATA IS ON THE RECORD · ".repeat(3)}
                </textPath>
              </text>
            </svg>
            <img src={mark} alt="" className={styles.sealMark} />
          </div>
        </div>
      </div>
    </section>
  );
};

export default PrivacyHero;
